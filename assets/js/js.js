...
function eventListenersLoader(){
    ...
    //! delete item car
    addEventListener('click', deleteProductCar); 
...
function carElementsHTML() {
    ...
    carCounter.innerText = carProducts.length
}

// hacer funcionar el boton de eliminar producto del carrito

function deleteProductCar(e) {
    if(e.target.classList.contains('delete__product')) {
        const productId = e.target.getAttribute('data-id');
        // Filtrar los productos diferentes al que queremos eliminar.
        carProducts = carProducts.filter(product => product.id !== productId);
        carElementsHTML(); // Re-imprimir la lista de productos en el carrito.
    }
}
